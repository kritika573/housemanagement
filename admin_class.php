...
Class Action {
	...
	function save_user(){
        ...
        if(empty($id)){
            $save = $this->db->query("INSERT INTO users(name,username,password,type) value ('{$name}','{$username}','{$password}','{$type}')");
            return 1;
        }else{			
			$save = $this->db->query("UPDATE users set name='$name',username='{$username}',password='{$password}',type='{$type}' where id = ".$id);
            return 3;
        }
	}
	...
	function notification(){
		extract($_POST);
		$data = " notifications_name = '".$name."' ";
		$data .= ", message = '$message' ";
	     
		$save = $this->db->query("INSERT INTO comments set ".$data);
		$id=$this->db->insert_id;
		if($save) {
			$query = $this->db->query("SELECT * FROM users WHERE type = 2")->fetch_all(MYSQLI_ASSOC);

			foreach ($query as $user) {
				$data1= '';
				$data1 = " flat_owner_id = '".$user['id']."' ";
				$data1 .= ", notification_id = '$id' ";
			    $this->db->query("INSERT INTO flat_owner_notification_assoc set ".$data1);	
			}
			return 1;
        }
	}
